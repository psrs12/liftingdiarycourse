...
interface WorkoutData {
  ...
  exercises: {
    id: number;
    name: string;
    sets: { id: number; reps: string; weight: string; rpe: string }[];
  }[];
}
...
export function EditWorkoutClient({ workout }: { workout: WorkoutData }) {
  ...
  const [exercises, setExercises] = useState<ExerciseForm[]>(
    workout.exercises.length > 0
      ? workout.exercises.map((e) => ({
          id: e.id,
          name: e.name,
          sets: e.sets.length > 0 ? e.sets : [emptySet()],
        }))
      : [emptyExercise()]
  );
  ...
  return (
    <main className="flex-1 p-6 max-w-3xl mx-auto w-full">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold font-sans">Edit Workout</h1>
      ...
      <div className="space-y-6">
        ...
        <Button
          ...
          disabled={isPending}
        >
          {isPending ? "Saving..." : "Update Workout"}
        </Button>
      </div>
    </main>
  );
